...
Connection.prototype.createCollection = function createCollection(collectionName, definition, cb) {
  ...
  if(collection.superClass=='E'){
    for(var cl in self.collections){
      if((collection.tableName == self.collections[cl].tableName)&&self.collections[cl].databaseClass){
        return cb(null, collection.schema);
      }
    }
  }
  //console.log(collectionName);
  if(collection.superClass!='V'&&collection.superClass!='E'){
    var parentClass = self.collections[collection.superClass];
    //console.log(parentClass.databaseClass);

    if(parentClass.databaseClass==undefined||parentClass.databaseClass==null){
      self.createCollection(collection.superClass,definition,cb);
      self.collections[collection.superClass].databaseClass = true;
    }
  }
  ...
  self.db.class.create(collection.tableName, collection.superClass)
    .then(function(klass) {
      ...
      self.collections[collectionName].databaseClass = klass;
      //console.log(collection.tableName);
      self._collectionSync.modifiedCollections.push(collection);
      var props = [];
      if(collection.superClass!='V'&&collection.superClass!='E'){
        var parentProps = self.collections[collection.superClass].orientdbSchema;
        var diff = {};
        for(var p in collection.orientdbSchema){
          if(!parentProps[p]) diff[p] = collection.orientdbSchema[p];
        }
        //console.log(_.values(diff));
        props = _.values(diff);
      }else{
        // Create properties
        props = _.values(collection.orientdbSchema);
      }
...
Connection.prototype.drop = function (collectionName, relations, cb) {
  ...
  for(var cl in self.collections){
    if(collectionName == self.collections[cl].superClass){
      self.drop(cl,relations,cb);
    }
  }
  if(collection.databaseClass!=null&&collection.database!=undefined){
    collection.drop(relations, cb);
    collection.databaseClass = null;
  }
